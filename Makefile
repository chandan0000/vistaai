...
db-init: docker-db
	...
	@sleep 3
	uv run --directory backend vistaai db upgrade
	@echo ""
	@echo "✅ Database initialized!"

db-migrate:
	@read -p "Migration message: " msg; \
	uv run --directory backend vistaai db migrate -m "$$msg"

db-upgrade:
	uv run --directory backend vistaai db upgrade

db-downgrade:
	uv run --directory backend vistaai db downgrade

db-current:
	uv run --directory backend vistaai db current

db-history:
	uv run --directory backend vistaai db history

# === Server ===
run:
	uv run --directory backend vistaai server run --reload

run-prod:
	uv run --directory backend vistaai server run --host 0.0.0.0 --port 8000

routes:
	uv run --directory backend vistaai server routes

# === Users ===
create-admin:
	@echo "Creating admin user..."
	uv run --directory backend vistaai user create-admin

user-create:
	uv run --directory backend vistaai user create

user-list:
	uv run --directory backend vistaai user list

# === Celery ===
celery-worker:
	uv run --directory backend vistaai celery worker

celery-beat:
	uv run --directory backend vistaai celery beat

celery-flower:
	uv run --directory backend vistaai celery flower
	@echo ""
...
docker-db:
	...
	@echo "   Connection: postgresql://postgres:postgres@localhost:5432/vistaai"
...
help:
	@echo ""
	@echo "vistaai - Available Commands"
	@echo "======================================"
	@echo ""
	@echo "Setup:"
